...
from scripts.easyeda_import.interaction import prompt_text, prompt_yes_no


def test_prompt_text_uses_prompt_toolkit_in_tty(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("scripts.easyeda_import.interaction.prompt_toolkit_prompt", lambda prompt: "typed")
    monkeypatch.setattr("scripts.easyeda_import.interaction.sys.stdin.isatty", lambda: True)
    monkeypatch.setattr("scripts.easyeda_import.interaction.sys.stdout.isatty", lambda: True)
...
def test_prompt_text_falls_back_to_input_without_prompt_toolkit(
    ...
    monkeypatch.setattr("scripts.easyeda_import.interaction.prompt_toolkit_prompt", None)
    monkeypatch.setattr("builtins.input", lambda prompt: "fallback")
...
def test_prompt_yes_no_accepts_default(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("scripts.easyeda_import.interaction.prompt_text", lambda prompt: "")
